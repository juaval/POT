...
if [ ! -f "${sample_name}_dereplicated/checked_check.txt" ]; then
    ...
    awk -F ',' '{OFS=",";print $3,$1,$2}' ${sample_name}_dereplicated/checkm2_info_temp.csv > ${sample_name}_dereplicated/checkm2_info.csv #hacky, but reorder columns to mimic dRep desired formatting
    touch ${sample_name}_dereplicated/checked_check.txt
fi

# Correr dREP
if [ ! -f "${sample_name}_dereplicated/dereplicate_check.txt" ]; then
    dRep dereplicate ${sample_name}_dereplicated -p 64 -g ../../POT/05_AVERAGING/${sample_name}_binetted/${sample_name}_binette_out/final_bins/*.fa --genomeInfo ${sample_name}_dereplicated/checkm2_info.csv
