...
config=slurm_configs/POT_slurm_config4-2.csv
...
for sample_name in "${SAMPLES_TO_PROCESS[@]}"; do
  ...
  if [ ! -f "${sample_type}/${sample_type}_maps/${sample_name}_build_check.txt" ]; then
    bowtie2-build ${concatenated_contigs} ${index_name}
  ...
  if [ ! -f "${sample_type}/${sample_type}_maps/${sample_name}_map_check.txt" ]; then
    bowtie2 --threads 32 -x ${index_name} -1 ${reads_file1}  -2 ${reads_file2}  -U ${reads_file3} -S ${result_dir}
...
concoct -t 32 --composition_file ${sample_type}/${sample_type}_contigs_10K.fa --coverage_file ${sample_type}/${sample_type}_coverage_table.tsv -b ${sample_type}/concoct_output/
